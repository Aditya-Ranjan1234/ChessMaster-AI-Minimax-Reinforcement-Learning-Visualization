...
class MainMenu(object):
  ...
  def create_window(self):
    single_frame = tk.Frame(self.root, bg="#DEB887")  # Light brown
    multi_frame = tk.Frame(self.root, bg="#DEB887")   # Light brown
    ...
    single_player = tk.Button(self.root, text="Single Player", command=self.single_player, bg="#8B4513", fg="white")
    single_player.place(x=50, y=100, height=40, width=100)

    local_multiplayer = tk.Button(self.root, text="Local Multiplayer", command=self.multiplayer, bg="#8B4513", fg="white")
...
class SinglePlayer(object):
  def __init__(self):
    self.root = tk.Tk()
    self.root.configure(background="#DEB887")  # Light brown
  ...
  def create_window(self):
    main_menu = tk.Button(self.root, text="Main Menu", command=self.main_menu, bg="#8B4513", fg="white")
    ...
    if self.color == "":
      white_king = tk.Button(self.root, bg="#DEB887", image=self.white_king_image,
                             command=lambda color="White": self.select_color(color))
      black_king = tk.Button(self.root, bg="#DEB887", image=self.black_king_image,
                             command=lambda color="Black": self.select_color(color))

    elif self.color == "White":
      white_king = tk.Button(self.root, bg="#8B4513", image=self.white_king_image,
                             command=lambda color="White": self.select_color(color))
      black_king = tk.Button(self.root, bg="#DEB887", image=self.black_king_image,
                             command=lambda color="Black": self.select_color(color))
      ...
      white_king = tk.Button(self.root, bg="#DEB887", image=self.white_king_image,
                             command=lambda color="White": self.select_color(color))
      black_king = tk.Button(self.root, bg="#8B4513", image=self.black_king_image,
                             command=lambda color="Black": self.select_color(color))

    white_king.place(x=60, y=140, height=64, width=64)
    black_king.place(x=275, y=140, height=64, width=64)

    play = tk.Button(self.root, text="Play", command=self.play, bg="#8B4513", fg="white")
...
def multiplayer_game(color, theme):
  ...
  pygame.display.set_caption("Chess Minimax Visualizer")
  chess_game = Game(game_window, color, 0)  # Use theme 0 (brown theme)
...
def single_player_game(color, theme, depth):
  pygame.init()
  game_window = pygame.display.set_mode((width, height))
  pygame.display.set_caption(f"Chess Minimax Visualizer - (AI Depth - {depth})")
  chess_game = Game(game_window, color, 0, depth)  # Pass depth here
  chess_game.board.initiate_pieces()
  fps = 60
  clock = pygame.time.Clock()
  running = True
